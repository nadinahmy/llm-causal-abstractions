...
| 2026-07-04 | `20260704` | **JMLR paper, continued — the intervention-algebra representation theorem.** Definitions 17–18, Lemma 19, and **Theorem 20**. The free algebra `A*` of intervention "to-do lists" under concatenation; the rewrite relation `≈` generated by **swap** (reorder edits to different variables, i.e. commutativity) and **delete** (a later edit to the same variable annihilates the earlier one, i.e. left-annihilativity); **normal forms** via Collapse (keep only the last edit per variable) and Sort; and the representation theorem proving the quotient `A*/≈` is an intervention algebra isomorphic to hard interventions `(Φ, ∘)` for a signature manufactured out of `A` itself. Includes the full proof (well-defined, surjective, injective homomorphism) and a worked example (`a: X:=3`, `b: X:=5`, `c: Y:=7`), plus a handwritten proof/example (*Theorem 20 — Proof and Example*). |
| 2026-07-05 | `JMLR - Causal Abstraction - Theorem 21` | **Theorem 21** — the closure `Ψ` of atomic **soft** interventions under composition is likewise an intervention algebra: soft interventions obey the same two laws (a pre-chosen mechanism-replacement overwrites blindly, so it leaves no fingerprint → left-annihilativity), so Theorem 20's construction transfers, yielding the manufactured signature `Σ*` whose meta-variable `X*` answers "*which mechanism is installed in slot X?*" and whose **values are the interventions themselves** (`Val_{X*} = Ψ₀^X`); with a running example (`Ψ₀ = {J, I₁, I₂}`) and a refresher on **isomorphism**. **Definition 24** — the induced ordering on an intervention algebra, `λ ≤ λ′ iff λ′ ⊕ λ = λ′` (the smaller intervention, applied last, is redundant), which recovers the natural partial order on hard interventions and is a semi-lattice, noting `⊕` is not commutative in general. Includes scanned handwritten notes.